 ...
 * project as a global npm package.  The tests then executes `generate-project` in a test directory (tempTestDir)
 * This is the most comprehensive way to verify that everything works before publishing to NPM.
 */
...
describe("ts-node-docker", () => {
  beforeAll(() => {
    executeCLI({ ...testConfig, template: "ts-node-docker" });
  });
